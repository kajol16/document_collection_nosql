...
def createDepartmentsDocument():
    ...
    results=database1.department.aggregate([
        ...
        {"$group": {"_id": "$Dnumber","Dname": {"$first":"$Dname"},"MGR_Lname": {"$first":"$R.Lname"},"MGR_Fname": {"$first":"$R.Fname"},"Mgr_start_date": {"$first":"$D.mgr_start_date"},
                    "employees": {"$push": {"E_Lname": "$S.Lname","E_Fname": "$S.Fname", "Salary": "$S.Salary"}}}},
